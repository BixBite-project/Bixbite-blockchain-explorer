...
struct tx_details
{
    ...
    get_mstch_map() const
    {
        ...
        if (!input_key_imgs.empty())
        {
            ...
            fee_str          = fmt::format("{:0.4f}", xmr_amount);
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs"        , xmr_amount_to_str(xmr_inputs , "{:0.4f}")},
                {"sum_outputs"       , xmr_amount_to_str(xmr_outputs, "{:0.4f}")},
                {"sum_inputs_short"  , xmr_amount_to_str(xmr_inputs , "{:0.4f}")},
                {"sum_outputs_short" , xmr_amount_to_str(xmr_outputs, "{:0.4f}")},
...
public:
    ...
    index2(uint64_t page_no = 0, bool refresh_page = false)
    {
       ...
       // uint64_t height = core_storage->get_current_blockchain_height();
        ...
        {
            hash_rate = fmt::format("{:0.4f} MH/s", current_network_info.hash_rate/1.0e6);
        }
        ...
        if (CurrentBlockchainStatus::is_thread_running())
        {
            ...
            string emission_coinbase = xmr_amount_to_str(current_values.coinbase, "{:0.4f}");
    ...
    show_block(uint64_t _blk_height)
    {
        ...
        context["sum_fees"]
                = xmreg::xmr_amount_to_str(sum_fees, "{:0.4f}", false);

        // get xmr in the block reward
        context["blk_reward"]
                = xmreg::xmr_amount_to_str(txd_coinbase.xmr_outputs - sum_fees, "{:0.4f}");
                    ...
                    bool tx_prove = false)
    {
        ...
        {
            return string("Bixbite address not provided!");
        }
        ...
        // parse string representing given Bixbite address
...
//        }
        ...
        {
            if (cryptonote::decrypt_payment_id(decrypted_payment_id8, pub_key, prv_view_key))
            {
...
//                                << endl;
    ...
    show_checkrawtx(string raw_tx_data, string action)
    {
        ...
        else
        {
            ...
            if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
            {
                ...
                // used in myBixbite and openBixbite projects.
    ...
    show_pushrawtx(string raw_tx_data, string action)
    {
        ...
        // if failed, treat raw_tx_data as base64 encoding of signed_Bixbite_tx
        else
        {
            string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
    ...
    search(string search_text)
    {
        ...
        // check if Bixbite address is given based on its length
        ...
        // check if integrated Bixbite address is given based on its length
 ...
 */
                 ...
                 bool tx_prove = false)
    {
        ...
        {
            j_response["status"]  = "error";
            j_response["message"] = "Bixbite address not provided";
            return j_response;
        }
        ...
        {
            j_response["status"]  = "error";
            j_response["message"] = "Cant parse Bixbite address: " + address_str;
            return j_response;
    ...
    json_networkinfo()
    {
        ...
        {
            j_response["status"]  = "error";
            j_response["message"] = "Cant get Bixbite network info";
            return j_response;
        }
...
private:
            ...
            secret_key const& prv_view_key)
    {
        ...
        if (decrypted_payment_id8 != null_hash8)
        {
            if (cryptonote::decrypt_payment_id(decrypted_payment_id8, txd.pk, prv_view_key))
            {
    ...
    construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
    {
        ...
        mstch::map context {
                ...
                {"tx_size"               , fmt::format("{:0.4f}", tx_size)},
                {"tx_fee"                , xmreg::xmr_amount_to_str(txd.fee, "{:0.4f}", false)},
                {"payed_for_kB"          , fmt::format("{:0.4f}", payed_for_kB)},
                {"tx_version"            , static_cast<uint64_t>(txd.version)},
                {"blk_timestamp"         , blk_timestamp},
