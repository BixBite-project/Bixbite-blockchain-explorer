...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
      ...
      /*  if (!parse_and_validate_tx_from_blob(
        ...
        last_tx.xmr_inputs_str   = xmreg::xmr_amount_to_str(last_tx.sum_inputs , "{:0.4f}");
        last_tx.xmr_outputs_str  = xmreg::xmr_amount_to_str(last_tx.sum_outputs, "{:0.4f}");
...
bf::path MempoolStatus::blockchain_path {"/home/mwo/.Bixbite/lmdb"};
string MempoolStatus::deamon_url {"http:://127.0.0.1:44051"};
